...
- **Content check:** before writing anything, it samples recent messages written by people and refuses to go on if most of them are blank, which is what a missing *Message Content Intent* looks like. A few blank messages, such as stickers or polls, do not stop it. An export made without the intent stays blank, because running it again does not refetch messages it already has: delete it and export again. If the messages really are blank, pass `--skip-preflight`.
